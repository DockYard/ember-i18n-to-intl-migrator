module.exports = function(file, api, options) {
  ...
  source
    ...
    .forEach(path => {
      path.node.properties.map(prop => {
        if (prop.key.name === 'i18n') {
          prop.key = j.identifier('intl');

          // service('i18n') --> service('intl')
          if (prop.value.arguments && prop.value.arguments.length === 1) {
  ...
  // import { translationMacro as t } from 'ember-i18n' to from 'ember-intl';
  let i18nImports = source.find(j.ImportDeclaration, {
    source: { value: 'ember-i18n' }
  }).forEach((importStatement) => {
    let newImport = [];
    importStatement.get('specifiers').each(({ node }) => {
      if (node.local && node.imported) {
        newImport.push({ [node.imported.name]: node.local.name });
      }
    });

    newImport.forEach((importObj) => {
      let imported = Object.keys(importObj)[0];
      let as = j.identifier(importObj[imported]);
      imported = j.identifier(imported);
      // { translationMacro as t }
      let existingSpecifier = j.importSpecifier(imported, as);
      let newImport = j.importDeclaration([existingSpecifier], j.literal('ember-intl'));
      j(importStatement).replaceWith(newImport);
    });
  });
